...
body{
  background:
    radial-gradient(1200px 600px at 80% -10%, #EAF0FF 0%, transparent 55%),
    radial-gradient(900px 500px at -10% 10%, #FFF1EF 0%, transparent 48%),
    var(--fondo);
...
Genera hoja de vida COMPLETA en texto profesional para imprimir. Incluye: encabezado, perfil impactante, experiencia con logros cuantificados, educación, habilidades, idiomas. Datos faltantes entre [COMPLETAR].`;
    try{
      const r=await fetch(AI_API_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6-20250514",max_tokens:2500,messages:[{role:"user",content:prompt}]})});
      const j=await r.json();
  ...
  const mejorar=async()=>{
    ...
    try{
      const r=await fetch(AI_API_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6-20250514",max_tokens:2500,messages:[{role:"user",content:"Mejora esta hoja de vida con verbos de acción más fuertes y logros más concretos:\n\n"+cv}]})});
...
function ModuloConversor({onVolver}){
  ...
  const procesar=async op=>{
    ...
    try{
      const r=await fetch(AI_API_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6-20250514",max_tokens:2000,messages:[{role:"user",content:prompts[op.id]+texto}]})});
      const j=await r.json();
