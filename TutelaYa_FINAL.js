...
:root{
  ...
  --alerta: #DC2626;
  --dorado: #F59E0B;
  --sombra-sm: 0 1px 3px rgba(11,42,107,.06), 0 1px 2px rgba(11,42,107,.04);
  --sombra-md: 0 4px 16px rgba(11,42,107,.08), 0 2px 6px rgba(11,42,107,.05);
  --sombra-lg: 0 12px 32px rgba(11,42,107,.12), 0 4px 12px rgba(11,42,107,.06);
  --sombra-azul: 0 8px 24px rgba(29,78,216,.28);
}
*{margin:0;padding:0;box-sizing:border-box;}
html{scroll-behavior:smooth;}
body{
  background:
    radial-gradient(1200px 600px at 80% -10%, #E8F0FF 0%, transparent 55%),
    radial-gradient(900px 500px at -10% 10%, #EAF2FF 0%, transparent 50%),
    var(--fondo);
...
nav{
  ...
  box-shadow:0 1px 0 rgba(11,42,107,.04);
}
...
.ba:hover{box-shadow:0 12px 30px rgba(29,78,216,.4);transform:translateY(-2px);}
.bo{
...
.bv{
  background:linear-gradient(135deg,var(--exito),#15803d);color:#fff;
  padding:13px 24px;font-size:14px;box-shadow:0 8px 24px rgba(22,163,74,.28);
}
.bv:hover{transform:translateY(-2px);box-shadow:0 12px 30px rgba(22,163,74,.4);}
.bd{
...
/* ── CAJAS DE ESTADO ──────────────────────────────────────────── */
.success-box{background:linear-gradient(135deg,#F0FDF4,#DCFCE7);border:1px solid #86EFAC;border-radius:14px;padding:14px 16px;color:#15803d;}
...
@keyframes glow{0%,100%{box-shadow:0 0 0 0 rgba(29,78,216,.4);}50%{box-shadow:0 0 0 12px rgba(29,78,216,0);}}
...
.dz:hover{border-color:var(--azul-claro);background:#E8F0FF;}
...
@media(max-width:760px){
  ...
  .cbot.activo{color:var(--azul);background:var(--azul-niebla);}
...
/* ── MODO OSCURO ──────────────────────────────────────────────── */
body.dark{
  --card-bg:#141B2E;--border2:#26314A;--text:#E2E8F0;
  --texto:#E2E8F0;--texto-suave:#94A3B8;--linea:#26314A;--fondo:#0A0F1C;
  --azul-niebla:#1A2335;--azul-cielo:#1E3A5F;
  background:
    radial-gradient(1200px 600px at 80% -10%, #15203A 0%, transparent 55%),
    radial-gradient(900px 500px at -10% 10%, #131C30 0%, transparent 50%),
    #0A0F1C;
}
body.dark nav{background:rgba(15,22,38,.85);}
body.dark .nav-inferior-movil{background:rgba(15,22,38,.92);}
body.dark .cc,body.dark .card{background:#141B2E;border-color:#26314A;}
body.dark .bo{background:#141B2E;color:#93C5FD;border-color:#1E3A5F;}
body.dark input,body.dark textarea,body.dark select{background:#0F1628;border-color:#26314A;color:#E2E8F0;}
body.dark .tdoc{background:#0F1628;color:#CBD5E1;border-color:#26314A;}
body.dark .tag-gray{background:#1E293B;color:#94A3B8;}
body.dark .dz{background:#131C30;border-color:#1E3A5F;}

.cb{background:linear-gradient(135deg,var(--azul-niebla),#E0E7FF);border:1px solid var(--azul-cielo);border-radius:14px;padding:14px 16px;}
...
Genera hoja de vida COMPLETA en texto profesional para imprimir. Incluye: encabezado, perfil impactante, experiencia con logros cuantificados, educación, habilidades, idiomas. Datos faltantes entre [COMPLETAR].`;
    try{
      const r=await fetch(API_IA,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${SUPABASE_ANON_KEY}`,"apikey":SUPABASE_ANON_KEY},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:2500,messages:[{role:"user",content:prompt}]})});
      const j=await r.json();
  ...
  const mejorar=async()=>{
    ...
    try{
      const r=await fetch(API_IA,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${SUPABASE_ANON_KEY}`,"apikey":SUPABASE_ANON_KEY},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:2500,messages:[{role:"user",content:"Mejora esta hoja de vida con verbos de acción más fuertes y logros más concretos:\n\n"+cv}]})});
...
function ModuloConversor({onVolver}){
  ...
  const procesar=async op=>{
    ...
    try{
      const r=await fetch(API_IA,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${SUPABASE_ANON_KEY}`,"apikey":SUPABASE_ANON_KEY},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:2000,messages:[{role:"user",content:prompts[op.id]+texto}]})});
      const j=await r.json();
